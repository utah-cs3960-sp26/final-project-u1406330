...
- HUD panels cover plot state, daily rewards, offline rewards, overview, and announcements, with a large bottom-left coin counter and Vault button as the primary player-facing HUD. A bubbly bottom-right Tutorial button now sits above the Resource Guide and opens a step-by-step how-to-play walkthrough for the mine, Vault, geode, and resource loop. Tutorial step cards now size to their actual text content instead of leaving large empty gaps between sections. The Resource Guide button opens a large two-tab guide: a visible color-coded rarity chart derived from the actual mining path for the currently equipped pickaxe, plus a collection page with one slot per resource ordered by rarity, shadowed unknown finds, and hover details for discovered resource attributes. The collection hover card now preserves the currently hovered slot across normal HUD refreshes instead of dropping back to the placeholder copy while the cursor is still resting on a bubble. The legacy Geode Inventory panel remains in the UI shell but is currently hidden.
...
- The mine and pickaxe shop facades now include larger server-authored seam fillers, and the shared cave shell side walls now overlap the north/south shell segments so the visible cave-wall gaps behind those structures stay closed without moving ownership out of `WorldReplicaService`.
...
- Players can sprint by holding Left Shift or Right Shift, which increases walk speed from 16 to 36. Speed resets on release and now reliably reapplies the current sprint state after respawn.
...
- A lightweight local geode reveal effect exists. Mining geode drops show a rarity-scaled discovery popup (bigger, bolder, longer duration at higher rarities) without rolling, while coin rewards now pop as lighter text-only `+x coins` bursts using the same bright yellow accent as the coin counter and a much larger center-screen spread instead of stacking in a narrow lane or using a boxed background. Cracking a geode now bursts open into a floating crystal-ore reveal model that reuses the resource silhouette language instead of the old placeholder orb, then triggers two sequential single-reel rolls: first the resource reel spins through actual resource names and lands on the base resource, then that popup closes and the attribute reel spins through attribute names if a non-none attribute was rolled. Resource and attribute names stay hidden until their reel finishes so the outcome is not revealed early, the resource reel now recolors live from each currently displayed resource instead of sticking to one fallback tone, and both the resource and attribute reels stay on screen for 3 seconds after settling so players have time to read the result. The geode shell and reveal effect now stay alive until the active celebration popup and any queued follow-up reel finish, so the crack animation no longer fades out before the roll sequence resolves. Reveal popup minimum heights now come from shared presentation rules so the resource-to-attribute handoff no longer flashes an oversized empty panel before the attribute reel settles, and the old click-footer handoff is no longer used for resource-to-attribute transitions. Local HUD audio now adds small discovery and reveal chimes tied to geode-find rarity plus cracked-resource and attribute reel reveals. The Geode Inventory HUD panel is hidden.
...
- Twenty-seven spec files currently live in `tests/`: EconomyMath, GeodeLifecycle, GeodePresentation, GeodeRoller, Geodes, GeodeState, GeodeToolService, MiningRewards, MiningService, OfflineMath, PlacementRules, RemoteNames, ResourceGuide, ResourceGuideTracking, Resources, ResourceVisuals, RewardPresentation, ShopCatalog, ShopRotation, SizeRoller, SizeRules, StationAssignment, TutorialGuide, VaultMath, VaultService, WorldLabelPresentation, and WorldReplicaService.
